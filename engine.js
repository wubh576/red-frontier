...
(function (root) {
  ...
  const cell = (x, y) =>
    clamp(Math.floor(y / TILE), 0, ROWS - 1) * COLS +
    clamp(Math.floor(x / TILE), 0, COLS - 1);
  const center = (i) => ({
    x: ((i % COLS) + 0.5) * TILE,
  ...
  class Game {
    constructor({
      ...
      this.visible = new Array(COLS * ROWS).fill(0);
      this.visionTimer = 0;
      this.navVersion = 0;
      this.navCache = null;
      this.generateMap();
      ...
      this.add("harvester", 0, 576, 1176);
      this.add("tank", 0, 432, 912);
      this.add("tank", 0, 528, 912);
      this.add("hq", 1, 1992, 264);
      this.add("power", 1, 2160, 192);
    ...
    add(type, team, x, y) {
      ...
      if (def.building) this.invalidateNav();
      return e;
    }
    get(id) {
    ...
    owned(type, team = 0) {
      return this.entities.filter(
        (e) => e.team === team && e.hp > 0 && (!type || e.type === type),
      );
    }
    invalidateNav() {
      this.navVersion++;
    ...
    power(team = 0) {
      return this.owned(null, team).reduce(
        (p, e) => {
          p.output += TYPES[e.type].output || 0;
          p.used += TYPES[e.type].draw || 0;
          return p;
        },
        { output: 0, used: 0 },
      );
    ...
    powered(team = 0) {
      ...
      return p.output >= p.used;
    }
    event(text, kind = "info") {
      this.events.push({ text, kind });
    ...
    blocked() {
      ...
      for (const e of this.entities)
        if (e.hp > 0 && TYPES[e.type].building) {
    ...
    snap(type, x, y) {
      const d = TYPES[type];
      return {
        x: (Math.floor(x / TILE - d.w / 2) + d.w / 2) * TILE,
        y: (Math.floor(y / TILE - d.h / 2) + d.h / 2) * TILE,
      };
    }
    canPlace(type, x, y) {
      const d = TYPES[type];
      if (!d || !d.building || type === "hq") return false;
      const b = {
        x: x - (d.w * TILE) / 2,
        y: y - (d.h * TILE) / 2,
        w: d.w * TILE,
      ...
      if (
        !this.owned().some(
          (e) => TYPES[e.type].building && dist(e, { x, y }) < 340,
        )
      )
        return false;
      const grid = this.blocked();
      for (
        let cy = Math.floor(b.y / TILE);
        cy < Math.ceil((b.y + b.h) / TILE);
        cy++
      )
        for (
          let cx = Math.floor(b.x / TILE);
          cx < Math.ceil((b.x + b.w) / TILE);
          cx++
        )
          if (grid[cy * COLS + cx] || !this.visible[cy * COLS + cx])
            return false;
      if (
        this.ore.some(
          (o) =>
    ...
    cancel(kind, index = 0) {
      ...
      if (!q || !q[index]) return;
      this.money[0] += TYPES[q[index].type].cost;
      q.splice(index, 1);
    }
    place(x, y) {
      const item = this.queue.building[0];
      if (!item || item.progress < 1) return false;
      const p = this.snap(item.type, x, y);
      if (!this.canPlace(item.type, p.x, p.y)) return false;
      const e = this.add(item.type, 0, p.x, p.y);
      this.queue.building.shift();
      if (e.type === "refinery") this.spawnAt("harvester", 0, e);
      this.updateVision();
      this.event(`${TYPES[e.type].name}已部署`, "build");
      return e;
    }
    nearestFree(x, y) {
      const grid = this.blocked();
      const start = cell(x, y);
      if (!grid[start]) return start;
      const sx = start % COLS,
      ...
      for (let r = 1; r < Math.max(COLS, ROWS); r++) {
        let best = -1,
          score = Infinity;
        for (
          let cy = Math.max(0, sy - r);
          cy <= Math.min(ROWS - 1, sy + r);
          cy++
        )
          for (
            let cx = Math.max(0, sx - r);
            cx <= Math.min(COLS - 1, sx + r);
            cx++
          ) {
            if (
              Math.max(Math.abs(cx - sx), Math.abs(cy - sy)) !== r ||
              grid[cy * COLS + cx]
            )
              continue;
            const ds =
              (cx * TILE + TILE / 2 - x) ** 2 + (cy * TILE + TILE / 2 - y) ** 2;
            if (ds < score) {
              best = cy * COLS + cx;
              score = ds;
            }
    ...
    spawnAt(type, team, building) {
      const b = this.bounds(building);
      const p = center(this.nearestFree(building.x, b.y + b.h + TILE));
      return this.add(type, team, p.x, p.y);
    }
    pathfind(from, to) {
      const grid = this.blocked(),
        start = cell(from.x, from.y),
        goal = this.nearestFree(to.x, to.y);
      if (start === goal) return [center(goal)];
      const open = [start],
        g = new Float64Array(COLS * ROWS).fill(Infinity),
        f = new Float64Array(COLS * ROWS).fill(Infinity);
      const parent = new Int32Array(COLS * ROWS).fill(-1),
        closed = new Uint8Array(COLS * ROWS);
      const h = (i) =>
        Math.hypot(
          (i % COLS) - (goal % COLS),
          Math.floor(i / COLS) - Math.floor(goal / COLS),
        );
      g[start] = 0;
      f[start] = h(start);
      while (open.length) {
        ...
        for (let dy = -1; dy <= 1; dy++)
          for (let dx = -1; dx <= 1; dx++) {
            ...
            if (
              ...
              continue;
            if (dx && dy && (grid[y * COLS + nx] || grid[ny * COLS + x]))
              continue;
    ...
    command(ids, kind, point, targetId) {
      const units = ids
        .map((id) => this.get(id))
        .filter((e) => e && e.team === 0 && !TYPES[e.type].building);
      ...
      units.forEach((e, i) => {
        ...
        e.order = {
          kind,
          x: clamp(
            point.x + (offset ? ((i % width) - (width - 1) / 2) * 42 : 0),
            24,
            WIDTH - 24,
          ),
          y: clamp(
            point.y +
              (offset
                ? (Math.floor(i / width) -
                    (Math.ceil(units.length / width) - 1) / 2) *
                  42
                : 0),
            24,
            HEIGHT - 24,
    ...
    targetDistance(a, b) {
      ...
      const box = this.bounds(b);
      return Math.hypot(
        a.x - clamp(a.x, box.x, box.x + box.w),
        a.y - clamp(a.y, box.y, box.y + box.h),
      );
    }
    move(e, goal, dt, stopRange = 5) {
      if (dist(e, goal) < stopRange) {
        e.path = [];
        return true;
      }
      e.routeTimer -= dt;
      if (e.routeTimer <= 0 || e.pathVersion !== this.navVersion) {
        e.path = this.pathfind(e, goal);
        e.routeTimer = 1.4;
        e.pathVersion = this.navVersion;
      }
      if (!e.path.length)
        return dist(e, center(this.nearestFree(goal.x, goal.y))) < 10;
      const p = e.path[0],
        dx = p.x - e.x,
        dy = p.y - e.y,
        distance = Math.hypot(dx, dy),
        step = TYPES[e.type].speed * dt;
      if (distance > 0.1) {
        e.angle = Math.atan2(dy, dx);
        e.x += (dx / distance) * Math.min(step, distance);
        e.y += (dy / distance) * Math.min(step, distance);
        e.traveled += step;
      }
      if (distance <= step + 1) e.path.shift();
      return false;
    }
    harvest(e, dt) {
      const refineries = this.owned("refinery", e.team);
      if (!refineries.length) return;
      const refinery = refineries.sort((a, b) => dist(e, a) - dist(e, b))[0];
      if (
        e.cargo >= 300 ||
        (!this.ore.some((o) => o.amount > 0) && e.cargo > 0)
      ) {
        const b = this.bounds(refinery),
          drop = center(this.nearestFree(refinery.x, b.y + b.h + 24));
        if (dist(e, drop) < 45) {
          this.money[e.team] += e.cargo;
          if (!e.team)
            this.effects.push({
              type: "income",
              x: e.x,
        ...
        const deposits = this.ore
          .filter((o) => o.amount > 0)
          .sort((a, b) => dist(preferred, a) - dist(preferred, b));
        if (!deposits.length) return;
        const ore = deposits[0];
        if (dist(e, ore) < 68) {
          const amount = Math.min(65 * dt, ore.amount, 300 - e.cargo);
          e.cargo += amount;
          ore.amount -= amount;
          e.path = [];
          e.harvestTimer += dt;
    ...
    updateVision() {
      ...
      for (const e of this.owned()) {
        const r = TYPES[e.type].sight,
          cx = Math.floor(e.x / TILE),
          cy = Math.floor(e.y / TILE),
          n = Math.ceil(r / TILE);
        for (let y = Math.max(0, cy - n); y <= Math.min(ROWS - 1, cy + n); y++)
          for (
            let x = Math.max(0, cx - n);
            x <= Math.min(COLS - 1, cx + n);
            x++
          ) {
            const i = y * COLS + x;
            if (dist(e, center(i)) <= r) this.explored[i] = this.visible[i] = 1;
          }
      }
    }
    isVisible(e) {
      return e.team === 0 || !!this.visible[cell(e.x, e.y)];
    }
    tick(dt) {
      if (this.outcome || !(dt > 0)) return;
      dt = Math.min(dt, 0.1);
      this.time += dt;
      ...
      for (const kind of ["building", "unit", "infantry"]) {
        ...
        item.progress = Math.min(
          1,
          item.progress +
            (dt / TYPES[item.type].time) * (this.powered() ? 1 : 0.35),
        );
      ...
      for (const e of this.entities) {
        ...
        e.cooldown -= dt;
        if (e.type === "harvester") {
          if (e.order && e.order.kind !== "harvest") {
            if (e.order.kind !== "hold" && this.move(e, e.order, dt))
              e.order = null;
        ...
        let target =
          e.order && e.order.kind === "attack"
            ? this.get(e.order.target)
            : null;
        if (target && e.team === 0 && !this.isVisible(target)) {
          e.order = { kind: "attackmove", x: e.order.x, y: e.order.y };
          target = null;
        }
        ...
        if (!target && mayAcquire) {
          const searchRange =
            e.order?.kind === "attackmove" ? d.sight : d.range;
          const enemies = this.entities.filter(
            (v) =>
              v.hp > 0 &&
              v.team !== e.team &&
              (e.team !== 0 || this.isVisible(v)) &&
              this.targetDistance(e, v) < searchRange,
          );
          target = enemies.sort(
            (a, b) => this.targetDistance(e, a) - this.targetDistance(e, b),
          )[0];
        }
        if (target && this.targetDistance(e, target) <= d.range) {
          ...
          if (e.cooldown <= 0 && (!d.building || this.powered(e.team))) {
            ...
            this.effects.push({
              type: "muzzle",
              x: e.x + Math.cos(e.turretAngle) * 26,
              y: e.y + Math.sin(e.turretAngle) * 26,
              life: 0.12,
            });
          }
        } else if (!d.building) {
          if (target && e.order?.kind !== "hold")
            this.move(e, target, dt, d.range * 0.85);
          else if (e.order && e.order.kind !== "hold") {
            if (this.move(e, e.order, dt)) e.order = null;
          } else e.turretAngle = e.angle;
        }
      }
      // Soft separation keeps formations legible without blocking narrow routes.
      const grid = this.blocked(),
        units = this.entities.filter(
          (e) => e.hp > 0 && !TYPES[e.type].building,
        );
      for (let i = 0; i < units.length; i++)
        for (let j = i + 1; j < units.length; j++) {
          const a = units[i],
            b = units[j],
            distance = dist(a, b),
            min = (TYPES[a.type].radius + TYPES[b.type].radius) * 0.85;
          if (distance < min) {
            const angle =
                distance < 0.01
                  ? a.id * 2.399
                  : Math.atan2(a.y - b.y, a.x - b.x),
              force = Math.min((min - distance) * 0.5, dt * 35);
            for (const [e, sign] of [
              [a, 1],
              [b, -1],
            ]) {
              const x = clamp(
                  e.x + Math.cos(angle) * force * sign,
                  24,
                  WIDTH - 24,
                ),
                y = clamp(
                  e.y + Math.sin(angle) * force * sign,
                  24,
                  HEIGHT - 24,
                );
              if (!grid[cell(x, y)]) {
      ...
      for (const p of this.projectiles) {
        ...
        if (distance <= speed + 8) {
          ...
          if (target.hp <= 0) {
            ...
            if (target.team === 1) this.kills++;
            else this.event(`${TYPES[target.type].name}已被摧毁`, "warning");
          }
        } else {
          p.x += ((target.x - p.x) / distance) * speed;
          p.y += ((target.y - p.y) / distance) * speed;
        }
      }
      this.projectiles = this.projectiles.filter((p) => p.life > 0);
      this.entities = this.entities.filter((e) => e.hp > 0);
      if (!this.owned("hq", 1).length) this.outcome = "victory";
      else if (!this.owned("hq").length) this.outcome = "defeat";
    }
  }
  const api = {
    Game,
